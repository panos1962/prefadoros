...
var Pliromi = new function() {
	...
	this.pliromi = function() {
		...
		// κανονικά, αλλιώς η αξία της μπάζας προσαρμόζεται με βάση
		// το υπόλοιπο της κάσας και τον τρόπο πληρωμής τελευταίας
		// αγοράς που έχει καθοριστεί για το συγκεκριμένο τραπέζι.

		// Αρχικά θέτουμε την αξία της μπάζας για μέσα αγορές να είναι
		// όσο είναι η κανονική αξία της μπάζας.
		this.axiaBazasMesa = this.axiaBazas;

		// Αν το υπόλοιπο της κάσας δεν επαρκεί, τότε πρέπει να γίνουν
		// κάποιες προσαρμογές.
		if ((pexnidi.ipolipo > 0) && (this.axiaBazas > pexnidi.ipolipo)) {
			if (isPostel()) {
				switch (partida.postel) {
				case 1:	// Ανισόρροπη πληρωμή τελευταίας αγοράς (πιπεριά).
					// Εδώ η αγορά πληρώνεται μειωμένη εφόσον έχει
					// βγει και κανονικά αν μπει μέσα.
					this.axiaBazas = pexnidi.ipolipo;
					break;
				}
				// Η μοναδική άλλη περίπτωση είναι η "δίκαιη" πληρωμή τελευταίας
				// αγοράς (ντομάτα), όπου η πληρωμή γίνεται κανονικά και κατόπιιν
				// εμφανίζεται αρνητικό υπόλοιπο κάσας.
			}
			else {
				// Δεν έχει καθοριστεί αλλαγή στον τρόπο πληρωμής της τελευταίας
				// αγοράς, οπότε η πληρωμή γίνεται κανονικά με προσαρμογή στο
				// τρέχον υπόλοιπο τόσο για τις έξω αγορές, όσο και για τις μέσα.
				this.axiaBazas = pexnidi.ipolipo;
				this.axiaBazasMesa = this.axiaBazas;
			}
		}

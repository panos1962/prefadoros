...
if ($globals->trapezi->is_theatis()) {
	die('Δεν μπορείτε να απορρίψετε το αίτημα ως θεατής');
}
...
$query = "DELETE FROM `κίνηση` WHERE (`διανομή` = " .
	$dianomi . ") AND (`είδος` LIKE 'CLAIM')";
$globals->sql_query($query);
if (@mysqli_affected_rows($globals->db) < 1) {
	Prefadoros::xeklidose_trapezi(FALSE);
	die('Απέτυχε η απόρριψη του αιτήματος');
}

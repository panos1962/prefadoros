...
function dialogosYesNo(msg, func, x, y) {
	if (notSet(x)) {
		x = null;
	}
	if (notSet(y)) {
	...
	dialogos('<div class="dialogosBox">' + msg +
		'<div style="text-align: right;">' +
		'<div class="dialogosYesNo" ' +
		'onmouseover="dialogosEntono(this);" ' +
		'onmouseout="dialogosEntono(this, false);"' +
		'onclick="dialogosOff();' + func + ';">' +
		'Ναι</div><div class="dialogosYesNo" ' +
		'onmouseover="dialogosEntono(this);" ' +
		'onmouseout="dialogosEntono(this, false);"' +
		'onclick="dialogosOff();">Άκυρο</div></div></div>', x, y);
}

function dialogosEntono(obj, yes) {
	if (notSet(obj)) {
		return;
	}

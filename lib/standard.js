function init() {
	...
	setTimeout(function() {
		...
		p.innerHTML = '<a href="https://twitter.com/prefadorosTT" target="_blank">' +
			'<img src="' + globals.server + 'images/twitter.png" ' +
	...
	setTimeout(function() {
		...
		p.innerHTML = '<form target="_blank" action="https://www.paypal.com/' +
			...
			'<input type="image" src="https://www.paypalobjects.com/en_US/i/btn/' +
	...
	setTimeout(function() {
		...
		p.innerHTML = '<iframe id="facebookLike" src="http://www.facebook.com/plugins/' +
			'like.php?href=http%3A%2F%2Fwww.prefadoros.gr&amp;send=false&amp;' +
			'layout=button_count&amp;width=90&amp;show_faces=false&amp;' +
			'action=like&amp;colorscheme=light&amp;font&amp;height=35" ' +
			'scrolling="no" frameborder="0" style="margin-left: 1px; border: none; ' +
			'overflow: hidden; width: 90px; height: 20px;"></iframe>';
	}, 3000);
}
...
function sviseNodeVima(x, opacity, vima) {
	...
	try { x.style.opacity = opacity / 100; } catch(e) {};
	try { x.filters.alpha.opacity = opacity; } catch(e) {};
	opacity -= 10;
	setTimeout(function() { sviseNodeVima(x, opacity, vima); }, vima);
}

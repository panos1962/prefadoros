function init() {
	...
	setTimeout(function() {
		...
		if (isSet(p)) { p.innerHTML = '<form target="_blank" action="https://www.paypal.com/' +
		...
		p = getelid('facebook');
		if (isSet(p)) { p.innerHTML = '<iframe src="http://www.facebook.com/plugins/like.php?' +
			...
			'overflow: hidden; width: 90px; height: 20px;" onerror="alert(13);"></iframe>';
		}
	}, 10);
}

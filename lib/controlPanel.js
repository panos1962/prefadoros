var controlPanel = new function() {
	...
	this.kafedaki = function(e, ico) {
		...
		req.xhr.onreadystatechange = function() {
			controlPanel.getMottoCheck(req, ico);
		};
		req.send();
	};

	this.getMottoCheck = function(req, ico) {
	...
	this.activeRadioHTML = function() {
		var cl = 'controlPanelIcon';
		var x = getelid('activeRadio');
		if (isSet(x) && isSet(x.style) && isSet(x.style.visibility) &&
			(x.style.visibility == 'visible')) { cl += ' controlPanelPatimeno'; }
		return '<img id="activeRadioIcon" class="' + cl + '" src="' +
			globals.server + 'images/controlPanel/activeRadio.png" alt="" ' +
			'title="ActiveRadio Live!" onclick="controlPanel.activeRadio();" />';
	};

	this.activeRadioStationHTML = function(station, desc, title) {
		if (notSet(title)) { title = desc; }
		return '<div class="activeRadioStation" onclick="controlPanel.' +
			'activeRadio(\'' + station + '\', \'' + title + '\');">' +
			desc + '</div>';
	};

	this.activeRadio = function(station, title) {
		var x = getelid('activeRadio');
		if (notSet(x)) { return; }

		if (notSet(station)) {
			station = 'http://activeradio.gr:8000/activemp3.m3u';
		...
		if (x.innerHTML == '') {
			var html = '';
			html = '<embed id="activeRadioMediaPlayer" type="application/x-mplayer2" ' +
				'name="activeRadioMediaPlayer" ' +
				'pluginspage="http://www.microsoft.com/Windows/Downloads/' +
				'Contents/Products/MediaPlayer/" src="' + station + '" ' +
				'transparentatstart="1" autostart="1" animationatstart="0" ' +
				'showcontrols="true" showaudiocontrols="1" showpositioncontrols="0" ' +
				'autosize="1" showstatusbar="1" displaysize="false" ';
			if (isSet(title)) { html += 'title="ActiveRadio Live!" '; }
			html += 'style="width: 6.8cm; height: 2.0cm;" />';
			html += controlPanel.activeRadioStationHTML('http://activeradio.gr:8000/' +
				'activemp3.m3u', 'ActiveRadio', 'ActiveRadio live!');
			html += controlPanel.activeRadioStationHTML('http://tvradio.ert.gr/radio/' +
				'liveradio/asx/net.asx', 'Δεύτερο πρόγραμμα');
			html += controlPanel.activeRadioStationHTML('http://tvradio.ert.gr/radio/' +
				'liveradio/asx/trito.asx', 'Τρίτο πρόγραμμα');
			html += controlPanel.activeRadioStationHTML('http://tvradio.ert.gr/radio/' +
				'liveradio/asx/kosmos.asx', 'KOSMOS 93.6');
			html += controlPanel.activeRadioStationHTML('http://tvradio.ert.gr/radio/' +
				'liveradio/asx/era5.asx', 'Η φωνή της Ελλάδας');
			html += controlPanel.activeRadioStationHTML('http://realfm.live24.gr/' +
				'realfm?17740f8b423b405352c21f303ab87e34', 'RealFM');
			x.innerHTML = html;
		}
		...
		if (aftomato) {
			...
			if (x.style.visibility != 'visible') {
				...
				cl += ' controlPanelPatimeno';
			}
			else {
				x.style.visibility = 'hidden';
			}

			var x = getelid('activeRadioIcon');
			if (isSet(x)) { x.setAttribute('class', cl); }
		}
	};
	...
	this.defaultPanelHTML = function() {
		return '<img src="' + globals.server + 'images/controlPanel/default.png" ' +
	...
	this.xanaBazaInnerHTML = function() {
		if (controlPanel.xanaBazaOn) {
			return '<img src="' + globals.server + 'images/controlPanel/trexousaBaza.gif" ' +
				'class="controlPanelIcon" alt="" title="Απόκρυψη τελευταίας μπάζας" ' +
				controlPanel.mesaExoHTML() + 'onclick="controlPanel.trexousaBaza();" />';
		}
		else {
			return '<img src="' + globals.server + 'images/controlPanel/xanaBaza.gif" ' +
				'class="controlPanelIcon" alt="" title="Επανεμφάνιση τελευταίας μπάζας" ' +
				controlPanel.mesaExoHTML() + 'onclick="controlPanel.xanaBaza();" />';
		}
	};
	...
	this.xanaBaza = function() {
		...
		x.innerHTML = Gipedo.telefteaBazaHTML();
		x.style.zIndex = 10;
	...
	this.idioktisia = function(ico) {
		...
		req.xhr.onreadystatechange = function() {
			controlPanel.idioktisiaCheck(req, ico);
		};

		req.send('idioktisia=' + uri(isIdioktito() ? 'ΕΛΕΥΘΕΡΟ' : 'ΙΔΙΟΚΤΗΤΟ'));
	};

	this.idioktisiaCheck = function(req, ico) {
		if (req.xhr.readyState != 4) { return; }
		var rsp = req.getResponse();
		mainFyi(rsp);
		if (rsp) {

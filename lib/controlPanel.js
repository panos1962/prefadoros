...
var ActiveRadio = new function() {
	...
	this.showRadio = function(station) {
		...
		if (x.innerHTML == '') {
			x.innerHTML = this.stationHTML(station);
	...
	this.stationHTML = function(station) {
		var html = '';
		if (slist.hasOwnProperty(station)) {
			html = '<embed id="activeRadioMediaPlayer" type="application/x-mplayer2" ' +
				...
				'Contents/Products/MediaPlayer/" src="' + slist[station].url + '" ' +
	...
	this.epilogiHTML = function(station) {
		...
		return '<div class="activeRadioStation" onclick="ActiveRadio.epilogi(\'' +

controlPanel = new function() {
	...
	this.processClaim = function() {
		...
		dialogos.innerHTML = '<div>Παρακαλώ περιμένετε την απάντηση ' +
			'των αμυνομένων…</div>';
		dialogos = null;
	};
	...
	this.claim = function() {
		...
		dialogos = getelid('dialogos');
		if (notSet(dialogos)) { return; }

		var html = '';
		html += '<div>Τα φύλλα σας θα φανερωθούν και θα απαιτήσετε όλες ' +
			'τις υπόλοιπες μπάζες. Αυτό θέλετε, πράγματι;</div><br />';
		...
		dialogos.innerHTML = html;
		dialogos.style.display = 'inline';
		dialogos.style.top = '4.0cm';
		dialogos.style.zIndex = 1;
	};

var controlPanel = new function() {
	...
	this.partidaDisplay = function() {
		...
		if (isPartida() && notTheatis() && notDianomi() && dikeomaRithmisis()) {
			html += controlPanel.learnerHTML();
		}
		else {
	...
	this.defaultPanelHTML = function(rev) {
		...
		html += '<img src="' + globals.server + 'images/controlPanel/default.png" ' +
			'title="Βασικά εργαλεία" class="controlPanelIcon';
		if (rev) { html += ' anastrofo'; }
		html += '" alt="" onclick="controlPanel.display(\'default\');" />';
		return html;
	};
	...
	this.refreshKitapi = function() {
		...
		if (dianomi.length == dianomiKitapi) { return; }
		dianomiKitapi = dianomi.length;
		// Στον Opera όταν κλείνει το παράθυρο του κιταπιού, δεν ενημερώνεται
	...
	this.learnerHTML = function() {
		if (isLearner()) {
			var ico = 'renrael';
			var title = 'Να γίνει αγωνιστική';
		}
		else {
			ico = 'learner';
			title = 'Να γίνει εκπαιδευτική';
		}
		return '<img src="' + globals.server + 'images/controlPanel/' + ico + '.png" ' +
			'class="controlPanelIcon" alt="" title="' + title + '" ' +
			controlPanel.mesaExoHTML() + 'onclick="controlPanel.learner(this);" />';
	};

	this.learner = function(ico) {
		Sizitisi.sxolioFocus();
		if (notPartida()) { return; }
		ico.prevSrc = ico.src;
		ico.src = globals.server + 'images/working.gif';
		mainFyi('Αλλαγή αγωνιστικό/εκπαιδευτικό');

		var req = new Request('trapezi/learner');
		req.xhr.onreadystatechange = function() {
			controlPanel.energiaCheck(req, ico);
		};
...
var ActiveRadio = new function() {
	var slist = {
		...
		FMSC:	{
			url: 'http://webplayer.radioswissclassic.ch/?lang=en',
			tit: 'Swiss Classic',
			ext: true
		}
	};

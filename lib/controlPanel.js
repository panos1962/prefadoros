controlPanel = new function() {
	...
	this.bugFixHTML = function() {
		return '<img src="' + globals.server + 'images/controlPanel/bugFix.png" ' +
			...
			controlPanel.mesaExoHTML() +
			'onclick="Dedomena.schedule(true);" />';
	};
	...
	this.claimHTML = function() {
		...
		if (pexnidi.fasi != 'ΠΑΙΧΝΙΔΙ') { return ''; }
		if (pexnidi.bazaFilo.length != 0) { return ''; }
		if (pexnidi.bazaCount >= 9) { return ''; }
		return '<img src="' + globals.server + 'images/controlPanel/claim.png" ' +
			'class="controlPanelIcon" alt="" title="Δεν δίνω άλλη μπάζα!" ' +
			controlPanel.mesaExoHTML() + 'onclick="controlPanel.claim(this);" />';
	};

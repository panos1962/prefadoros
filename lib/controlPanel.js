var controlPanel = new function() {
	...
	this.activeRadioHTML = function() {
		...
		return '<img id="activeRadioIcon" class="' + attrs.cl + '" src="' +
			...
			'title="' + attrs.ti + '" onclick="ActiveRadio.epilogi();" />';
	};
...
var ActiveRadio = new function() {
	this.attributes = function(div) {
		...
		if (div.style.visibility == 'visible') {
			...
			attrs.ti = 'Απόκρυψη ραδιοφώνου';
		}
		return attrs;
	};

	this.epilogi = function(station, title) {
		...
		if (x.innerHTML == '') {
			...
			html += this.stationHTML('http://activeradio.gr:8000/' +
				'activemp3.m3u', 'ActiveRadio', 'ActiveRadio live!');
			html += this.stationHTML('http://tvradio.ert.gr/radio/' +
			...
			html += this.stationHTML('http://realfm.live24.gr/' +
		...
		var attrs = this.attributes(x);
		p.setAttribute('class', attrs.cl);
		p.setAttribute('title', attrs.ti);
	};

	this.stationHTML = function(station, desc, title) {
		if (notSet(title)) { title = desc; }
		return '<div class="activeRadioStation" onclick="ActiveRadio.' +
			'epilogi(\'' + station + '\', \'' + title + '\');">' +
			desc + '</div>';
	};
};

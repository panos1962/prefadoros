controlPanel = new function() {
	...
	this.akirosiKinisisHTML = function() {
		...
		html += '<img class="controlPanelIcon" alt="" ' + controlPanel.mesaExoHTML() +
			' src="' + globals.server + 'images/controlPanel/';
		if (pexnidi.akirosi == 0) {
			html += 'akirosiKinisis.png" title="Ακύρωση κινήσεων" ' +
				'onclick="controlPanel.akirosiKinisis(this);"';
		}
		else {
			html += 'anamoniAkirosis.png" title="Λήξη ακύρωσης κινήσεων" ' +
				'onclick="controlPanel.anamoniAkirosis(this);"';
		}
		html += ' />';
		return html;
	};
	...
	this.akirosiKinisis = function(img) {
		...
		mainFyi('Ακύρωση κίνησης');
		img.src = globals.server + 'images/working.gif';
		var req = new Request('trapezi/akirosiKinisis');
		req.xhr.onreadystatechange = function() {

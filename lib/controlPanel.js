controlPanel = new function() {
	...
	this.xanaBazaInnerHTML = function() {
		if (xanaBazaOn) {
			return '<img src="' + globals.server + 'images/controlPanel/trexousaBaza.png" ' +
				'class="controlPanelIcon" alt="" title="Επανεμφάνιση τρέχουσας μπάζας" ' +
				controlPanel.mesaExoHTML() + 'onclick="controlPanel.trexousaBaza();" />';
		}
		else {
			return '<img src="' + globals.server + 'images/controlPanel/xanaBaza.png" ' +
	...
	this.trexousaBaza = function() {
		...
		mainFyi('Επανεμφάνιση τρέχουσας μπάζας');
		x.style.zIndex = -1;
		xanaBazaOn = false;
	...
	this.refreshHTML = function() {
		return '<img src="' + globals.server + 'images/controlPanel/refresh.png" ' +
			...
			controlPanel.mesaExoHTML() +
			'onclick="location.href=(globals.server+\'?motd=no&amp;diafimisi=no\');" />';
	};
